...
const infoCards = [
  {
    ...
    content: (
      <dl className="grid grid-cols-2 gap-3 text-sm text-white/85">
        <dt className="text-white/50">Token Name</dt>
        <dd className="font-semibold">WOODY Meme</dd>
        <dt className="text-white/50">Symbol</dt>
        <dd className="font-semibold text-sky-300">$WOODY</dd>
        <dt className="text-white/50">Supply Model</dt>
        <dd className="font-semibold">Transparent</dd>
        <dt className="text-white/50">Contract</dt>
        <dd className="truncate font-semibold">WOODY-5f9d9c</dd>
      </dl>
    ),
  },
  {
    ...
    content: (
      <ul className="space-y-2 text-sm text-white/85">
        {monitorFeatures.map((item) => (
          <li key={item} className="rounded-lg border border-sky-400/20 bg-sky-400/5 px-3 py-2">
            {item}
          </li>
        ))}
      </ul>
  ...
  },
  {
    ...
    content: (
      <div className="space-y-3 text-sm">
        <p className="text-white/75">Designed for smooth trading access across major MultiversX venues.</p>
        <div className="flex flex-wrap gap-2">
          <span className="rounded-lg border border-emerald-400/40 bg-emerald-400/10 px-3 py-1.5 font-medium text-emerald-300">xExchange</span>
          <span className="rounded-lg border border-orange-400/40 bg-orange-400/10 px-3 py-1.5 font-medium text-orange-300">OneDex</span>
          <span className="rounded-lg border border-sky-400/40 bg-sky-400/10 px-3 py-1.5 font-medium text-sky-300">Aggregator-ready</span>
        </div>
      </div>
    ),
  },
      ...
      <ol className="space-y-2 text-sm text-white/85">
        {roadmap.map((step, index) => (
          <li key={step} className="rounded-lg border border-white/10 bg-white/5 px-3 py-2">
            <span className="mr-2 font-semibold text-orange-300">0{index + 1}</span>
            {step}
          </li>
        ))}
      </ol>
    ...
    <main className="mx-auto flex w-full max-w-7xl flex-col gap-10 px-4 py-8 md:gap-14 md:px-8 md:py-12">
      <section className="card relative overflow-hidden p-6 shadow-[0_0_50px_rgba(59,130,246,0.16)] md:p-10">
        ...
        <div className="relative z-10 grid items-center gap-8 lg:grid-cols-[1.2fr_0.8fr]">
          <div>
            <p className="mb-4 inline-flex rounded-full border border-emerald-400/40 bg-emerald-400/10 px-4 py-1 text-xs font-semibold uppercase tracking-[0.2em] text-emerald-300">
              Crypto Meme Ecosystem
            </p>
            <h1 className="text-4xl font-black leading-tight text-white md:text-6xl">
              WOODY Meme
              <span className="mt-2 block text-2xl text-sky-300 md:text-4xl">Built for Community, Speed & Hype</span>
            </h1>
          ...
          <div className="rounded-2xl border border-white/10 bg-[#0b1324]/80 p-5">
            <h2 className="text-lg font-bold text-white">WOODY Pulse</h2>
